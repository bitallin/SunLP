...
class NgramModel:
    ...
    def get_ngram_freq_info(sentences: iter,
        ...
        def _process_corpus_chunk(sentence_chunk: list):
            ...
            for ni in [1] + list(range(min_n, max_n + 2)):
                ngram_generator = NgramModel.generate_ngram(sentence_chunk, ni)
        ...
        for sentence in sentences:
            if len(sentence_chunk) < chunk_size:
                ...
                ngram_freq_total = NgramModel.union_word_freq(ngram_freq, ngram_freq_total)
                sentence_chunk = []
        # 最后一批的处理
        ngram_freq = _process_corpus_chunk(sentence_chunk)
        ngram_freq_total = NgramModel.union_word_freq(ngram_freq, ngram_freq_total)
    ...
    def _calc_ngram_entropy(
        ...
        if isinstance(n, Iterable):  # 一次性计算 len(N)>1 的 ngram
            entropy = {}
            for ni in n:
                entropy = {**entropy, **NgramModel._calc_ngram_entropy(ngram_freq, ngram_keys, ni)}
            return entropy
        ngram_entropy = {}
        ...
        for target_ngram in target_ngrams:
            try:  # 一定情况下, 一个candidate ngram 没有左右neighbor
                ...
                right_entropy = NgramModel._ngram_entropy_score(right_neighbor_counts)
            ...
            try:
                ...
                left_entropy = NgramModel._ngram_entropy_score(left_neighbor_counts)
            except KeyError:
                left_entropy = 0
    ...
    def _calc_ngram_pmi(
        ...
        if isinstance(n, Iterable):
            mi = {}
            for ni in n:
                mi = {**mi, **NgramModel._calc_ngram_pmi(ngram_freq, ngram_keys, ni)}
            return mi
    ...
    def get_scores(sentences: iter,
        ...
        # 先统计词频，和ngram对应的词典
        ngram_freq, ngram_keys = NgramModel.get_ngram_freq_info(sentences, min_n, max_n, chunk_size,
                                                                min_freq=min_freq)
        # 计算词的left_right_entropy: {'word':(left_entropy, right entropy)}
        left_right_entropy = NgramModel._calc_ngram_entropy(ngram_freq, ngram_keys, list(range(min_n, max_n + 1)))

        # 计算词的 PMI
        mi = NgramModel._calc_ngram_pmi(ngram_freq, ngram_keys, range(min_n, max_n + 1))
        ...
        word_info_scores = {word: (mi[word][0],  # point-wise mutual information
                                   ...
                                   # branch entropy  BE=min{left_entropy,right_entropy}
                                   word_liberalization(
